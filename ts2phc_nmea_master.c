...
static struct timespec ts2phc_nmea_master_getppstime(struct ts2phc_master *master)
{
	struct ts2phc_nmea_master *m =
		container_of(master, struct ts2phc_nmea_master, master);
	...
	t1 = timespec_to_tmv(m->local_monotime);
	t2 = timespec_to_tmv(now);
	rmc = tmv_add(rmc, tmv_sub(t2, t1));

	t1 = timespec_to_tmv(m->rmc_utctime);
	t2 = timespec_to_tmv(m->local_utctime);
	pr_debug("nmea delay: %" PRId64 " ns",
		 tmv_to_nanoseconds(tmv_sub(t2, t1)));

	result = tmv_to_timespec(rmc);
	result.tv_sec += m->tai_offset;
